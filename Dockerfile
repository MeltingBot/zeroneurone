...
# Sync the third-party workers with the installed versions. `npx vite build`
# skips npm's prebuild hook, so without this the committed copies can lag behind
# the bundles in node_modules: pdf.js reports "API version X does not match the
# Worker version Y" at runtime, and MapLibre silently parses no GeoJSON source
# at all — no link lines, no zones, nothing in the console.
RUN npm run sync:workers
